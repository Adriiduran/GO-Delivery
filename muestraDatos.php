    ...
    <div class="container-fluid min-vh-100 flex-column d-flex">
        ...
        <div class="row flex-grow-1 bg-dark">
            ...
            <div class="col-10 d-flex flex-column align-items-center my-4 text-white">
                <h4 class="mb-4">CONSULTA DATOS POR FECHA <?= $_SESSION['fecha'][0] . ' / ' . $_SESSION['fecha'][1]; ?></h4>
                <?php
                if (empty($_SESSION['datos'])) {
                    ?>
                    <h5 class="mb-4 text-danger">NO HAY DATOS DISPONIBLES PARA LAS FECHAS SELECCIONADAS</h5>
                    <?php
                } else {
                ?>
                    <table class="table w-50 text-white border text-center">
                        <thead>
                            <tr class="bg-primary">
                                <th scope="col" class="border">Fecha</th>
                                <th scope="col" class="border">Turno</th>
                                <th scope="col" class="border">Tipo</th>
                                <th scope="col" class="border">Horas</th>
                                <th scope="col" class="border">Pedidos</th>
                                <th scope="col" class="border">LLuvia</th>
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr class="bg-light text-black">
                                    <th class="border" scope="row"><?= $dato['fecha'] ?></th>
                                    <td class="border"><?= $dato['turno'] ?></td>
                                    <td class="border"><?= $dato['tipo'] ?></td>
                                    <td class="border"><?= $dato['horas'] ?></td>
                                    <td class="border"><?= $dato['pedidos'] ?></td>
                                    <td class="border"><?= $dato['lluvia'] ?></td>
                                    <td class="border"><?= $dato['total'] ?></td>
                                </tr>
                            <?php
                            }
                            ?>
                ...
                ?>
                <a href="./consulta.php" class="ms-2 fs-5 text-decoration-none text-white btn btn-warning">Volver</a>
            </div>
        </div>
    </div>
